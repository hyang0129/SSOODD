...
def _get_filename_by_index(dataset, index):
    ...
    if isinstance(dataset, datasets.ImageFolder):
        ...
        return (index, os.path.relpath(full_path, dataset.root))
        ...
        return (index, dataset.get_filename(index))
    elif isinstance(dataset, MySubset):
        # filename is constructed by the video dataset
        return (index, dataset.indices[index])
    else:
...
class LightlyDataset:
    ...
    @classmethod
    def from_torch_dataset(cls, dataset, transform=None, target_transform=None, index_to_filename=None):
        ...
        if transform is not None:
            ...
            dataset_obj.target_type = target_transform
            # if target_transform is not None:
            #     dataset = datasets.wrap_datasets_for_transforms_v2(dataset, target_transform)
